...
const ChainLogo = ({ chain }: { chain: string }) => {
  ...
  return (
    <div
      className={`w-6 h-6 rounded-full bg-gradient-to-br ${getChainColor(
        chain
  ...
  const fetchNews = async (showLoading = true) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/news?lang=${language}&category=${selectedCategory}`,
        {
      ...
      if (error instanceof Error) {
        if (error.name === "AbortError") {
          // 사용자가 떠났거나, 타임아웃으로 취소된 경우: 메시지만 보여주고 치명적 오류로 취급하지 않음
          ...
          <div className="flex items-center space-x-3 text-gray-400">
            ...
            <span className="text-sm">
              {selectedCategory === "all"
                ? t.noNewsAvailable
                : `'${selectedCategory}' ${t.noNewsInCategory}`}
            </span>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
            ...
            <div className="flex items-center space-x-1 lg:space-x-2">
              ...
              {/* 속도 조절 - 팝오버로 변경 */}
              <Popover
                open={isSpeedPopoverOpen}
                onOpenChange={setIsSpeedPopoverOpen}
              >
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="text-xs text-gray-500 pt-2 border-t border-gray-700">
                      ...
                      <br />한 바퀴 시간: 약{" "}
                      {Math.round(getAnimationDuration(speed))}초
                    </div>
                  </div>
                </PopoverContent>
              </Popover>
          ...
          >
            {newsItems.length > 0 ? (
              <div
                className={`ticker-content ${isPlaying ? "" : ""} ${
              ...
              >
                ...
                {news.map((item, index) => (
                  ...
                  >
                    ...
                    <Badge
                      variant="secondary"
                      className="text-xs ml-1 lg:ml-2 hidden sm:inline-flex"
                    >
                      {item.source}
                    </Badge>
