...
export default function AIChatModal({
  ...
  const [usageStats, setUsageStats] = useState(
    chatLimitManager.getUsageStats()
  );
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 사용량 제한 확인
    if (
      !chatLimitManager.canMakeQuery() &&
      !subscriptionManager.canMakeQuery()
    ) {
    ...
    // 회원가입 유도 (3회 사용 후)
    if (
      usageStats.dailyQuestions >= 2 &&
      !usageStats.isRegistered &&
                ...
                <div className="flex items-center space-x-2">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">
                      {usageStats.remainingQueries === -1
                        ? "∞"
                        : usageStats.remainingQueries}
                      회 남음
                    </span>
                  ...
                  {subscriptionStatus.tokens > 0 && (
                    <Badge
                      variant="outline"
                      className="flex items-center space-x-1 border-yellow-500 text-yellow-500"
                    >
        ...
        <div className="flex flex-col h-full space-y-4">
          ...
          {isLimitReached && (
            <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4">
              ...
              <p className="text-red-300 text-sm mt-2">
                다음 리셋까지:{" "}
                <span className="font-medium">
                  {chatLimitManager.getTimeUntilReset()}
                </span>
              </p>
          ...
          <div className="flex items-center justify-between text-xs text-gray-500">
            <div className="flex items-center space-x-2">
              ...
              <span>
                {!usageStats.isPremium &&
                  `다음 리셋: ${chatLimitManager.getTimeUntilReset()}`}
