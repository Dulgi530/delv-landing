...
async function fetchRSSFeed(feedConfig: {
  ...
  try {
    console.log(
      `RSS 피드 가져오기 시도: ${feedConfig.source} (우선순위: ${feedConfig.priority})`
    );
    ...
    if (!response.ok) {
      throw new Error(
        `HTTP ${response.status} ${response.statusText} for ${feedConfig.source}`
      );
    }
    ...
    if (!xmlText || xmlText.length < 100) {
      throw new Error(
        `${feedConfig.source}: RSS 내용이 너무 짧습니다 (${xmlText.length}자)`
      );
    }
    ...
    if (xmlText.includes("<!DOCTYPE html") || xmlText.includes("<html")) {
      throw new Error(
        `${feedConfig.source}: HTML 페이지를 반환함 (RSS 피드 아님)`
      );
    }
    ...
    if (!hasRssTag && !hasFeedTag) {
      throw new Error(
        `${feedConfig.source}: RSS 또는 Atom 태그를 찾을 수 없음`
      );
    }
    ...
    const newsItems: NewsItem[] = feed.items
      ...
      .map((item: any, index: number) => {
        ...
        return {
          id: `${feedConfig.source
            .toLowerCase()
            .replace(/\s+/g, "-")}-${index}-${Date.now()}`,
          chain,
  ...
  try {
    ...
    const response = await fetch(
      `https://newsapi.org/v2/everything?q=${encodeURIComponent(
        query
      )}&language=${langParam}&sortBy=publishedAt&pageSize=15&apiKey=${apiKey}`,
      {
        headers: {
          "User-Agent": "DELV-CHAT-News-Bot/1.0",
        },
    ...
    const newsItems = data.articles
      ...
      .map((article: any, index: number) => {
        const chain = extractChainFromNews(
          article.title,
          article.description || ""
        );
  ...
  if (language === "ko") {
    ...
    return [
      ...
      },
      {
        ...
        chain: "Ethereum",
        title:
          "Ethereum Layer 2 Solutions See 300% Growth in Transaction Volume",
        description:
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (
      cachedNews[cacheKey] &&
      now - cachedNews[cacheKey].timestamp < CACHE_DURATION
    ) {
    ...
    primaryResults.forEach((result, index) => {
      if (result.status === "fulfilled" && result.value.length > 0) {
        ...
        console.log(
          `${primaryFeeds[index].source}: ${result.value.length}개 뉴스 추가`
        );
      } else {
    ...
    return Response.json({
      ...
      debug: {
        ...
        rssNews: finalNews.filter(
          (item) =>
            !item.id.startsWith("newsapi") && !item.id.startsWith("fallback")
        ).length,
        newsApiNews: finalNews.filter((item) => item.id.startsWith("newsapi"))
          .length,
        fallbackNews: finalNews.filter((item) => item.id.startsWith("fallback"))
          .length,
        sourceStats,
        language,
        category,
